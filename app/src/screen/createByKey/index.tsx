...
export default ({navigation}: any) => {
  ...
  const [prvSecureText, setPrvSecureText] = React.useState(true);
  const [eyePrvState, setEyePrvState] = React.useState(false);
  ...
  const eysPrvClick = () => {
    setEyePrvState(!eyePrvState);
    setPrvSecureText(!prvSecureText);
  }
  ...
  const createWallet = async () => {
    try {
      ...
      if (!privateKey) return Alert.alert("私钥不能为空!");
      ...
      const createWallet: any = ChainUtils.createWalletFormPrivateKey(privateKey);
      const wallet: Wallet = {
        name: account,
        mnemonic: "",
    ...
    <View style={styles.container}>
      <ScrollView style={styles.scrollview}>
        ...
        <Input
          ...
          label="私钥"
          placeholder="输入私钥"
          secureTextEntry={prvSecureText}
          rightIcon={
            // "eye-outline"
            <Icon type="ionicon" onPress={eysPrvClick} size={18}
                  name={eyeState ? "eye-outline" : "eye-off-outline"}/>
          }
        />
